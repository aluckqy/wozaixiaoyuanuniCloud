...
exports.main = async (event, context) => {
	...
	for (let user of getUserList.data) {
		if (user.jwsession) {
			...
			if (res.data.code != 0) {
				...
				const respw = await uniCloud.httpclient.request(
					'https://gw.wozaixiaoyuan.com/basicinfo/mobile/my/changePassword', {
						method: 'GET',
						data: {
				...
				if(respw.data.code != 0){
					cPassword[user.telephone]= respw.data
				}
			}
	...
	response["cPasswordlog"] = cPassword
